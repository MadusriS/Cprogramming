...
void handle_client(int client_socket) {
    ...
    // Extract the target server information from the request
    char target_host[256];  // Adjust the size as needed
    int target_port;

    // Parse the request to obtain target host and port
    if (sscanf(buffer, "Host: %255[^:\n]:%d", target_host, &target_port) != 2) {
        fprintf(stderr, "Error parsing target server information\n");
        close(client_socket);
        exit(EXIT_FAILURE);
    }
    ...
    target_address.sin_port = htons(8071);
